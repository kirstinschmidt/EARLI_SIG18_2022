...
- in our second study we again analyzed anchoring effects but this time we analyzed whether externally set anchors influence student teachers' interpretation of scientific evidence 
...
- To answer this research question, 233 student teachers from the KA University of Eduaction participated in our study
- as you can see just to get a quick impression of the demographics of our sample- most of the partipants were female and studied at least one STEM subject
...
- We conducted again an experimental study as in study 1.
- But in this study, participants were in total presented with two different study reports and were afterwards asked to rate the appropriateness of different features of the described scientific studies. 

- Let's take a closer look at what this exactly includes:
  - As I said, each student teacher was presented with two different study reports
  - Here you can see an example of such a study report on discovery learning. 
  - Both reports were structured identically: They first informed about the research question a group of educational researchers was interested in - here it was whether students learn better in a lesson with discovery learning than a teacher-centered lesson. Afterwards the reports informed about the experimental design of the study - more precisely a randomized controlled trial with one experimental and one control group but described in an easy language - and the drawn sample size - here e.g., a sample size of 15 participants per group. 
  - The reports not only randomly varied in their topic - the topics were discovery learning - as in this example - and learning styles. 
  - But also in the drawn sample size. We manipulated the drawn sample size in a way that either a small sample (15 participants per group) - as in this example - or a large sample of 500 participants was presented.
- This approach resulted in 
  ...
  - and two experimental groups, where student teachers were presented with different sample size in random order e.g., first 15, then 500 participants.
  
- Results obtained through the described experiment, were presented in a later step, but this part is irrelevant to the research question at hand which is why we skip this part today
...
- But what we want to look at is that after reading the study report, participants both in the ExperimentalGs and CGs had to answer different questions.
...
???
- Using these materials, we in general assumed that the sample size in study 1 influences the appropriateness ratings of the sample size in study 2. 

- More specifically, we formulated a hypothesis that consisted of two parts. Before I explain these two parts in more detail, just a quick reminder which group is which because this can be a bit confusing but is important to understand our hypothesis. You can see the reminder at the bottom of the slide, as well:
...
- We assumed that, taking into account the appropriateness rating of sample size 1, the experimental group 1 - the group with the small anchor - rates the presented sample size 2 as more appropriate than the experimental group 2 
...
- To test this hypothesis we conducted a Bayesian ANOVA by including the treatment groups as the independent variable and the so-called adjusted appropriateness of sample size 2 as the dependent variable. To create this dependent variable, we subtracted the appropriateness rating of the first study sample from the appropriateness rating of the second one.
...
- It is quite obvious that in line with our hypothesis, the experimental groups differ in their appropriateness ratings: EG 1 - in the bottom right corner - rates the sample size 2 as more appropriate than EG 2. It can be clearly seen that this difference indicates a large effect (d-EGs: 2.97)
- In contrast, the control groups in the left corner at the bottom and right corner at the top barely differ in their appropriateness ratings of sample size 1 and two 2 because the adjusted appropriateness ratings of sample size 2 are nearly zero. (d-CGs: 0.29). This result is again in line with our hypothesis
...
- In both studies, we found evidence for anchoring effects in the context of EIEP. While Study 1 shows evidence for anchoring effects on teacher students’ engagement with raw data <!-- passgt das noch so? -->, the second one presents evidence for anchoring effects on interpreting scientific evidence. 

- But why is it important to consider anchoring effects in the context of EIEP? <!-- Especially because anchor effects are well studied. -->
...
- I will illustrate the importance using two brief examples for DBDM and RIEP
...
- I will start with DBDM: Let's think of  benchmarks that are given in national tests, such as the mean value of a federal state. I think it is quite intuitive that these act as anchors because teachers compare the results of their own students with these benchmarks, which is of course intended by them. The comparison leads to different consequences, e.g. to different teaching practices. Such an Anchor can have positive consequences like facilitating an adequate interpretation of the students' performance but it can also lead to consequences that are not intended by statewide assessments. One such consequence is the example of teaching to the test if the performance of a class was poor.

- In the context of RIEP, large samples of large-scale assessments could act as an anchor in the evaluation of experimental studies. These large samples could lead to a devaluation of the quality and validity of experimental studies due to comparatively small samples and in turn, to less consideration of their findings in school practice.
- But we all know that sample size is not the only indicator of quality and validity.

<!-- noch irgendwie erwähnen, dass wir die Konsequenzen nicht untersucht haben, aber das die Bedeutsamkeit verdeutlicht? --> 
...
- It is important to better understand these biases in the context of RIEP to improve
decision-making either by relying on different types of data and scientific evidence
...
- mediator
- limitations 
- replikationskrise: wir meinen Dinge zu wissen, die sich dann aber z.B. in anderen Kontexten nicht replizieren lassen
---
